...
class cipherToolsUI(QWidget):
    ...
    def privkey_input(self):
        ...
        if True != self.is_hex_string(pvk_str):
            ...
            QMessageBox.about(self, "Error", "Please input hex number")
            return
    ...
    def sm4_encrypt(self):
        ...
        if self.pad == True:
            if self.mode == "ECB":
                ...
                QMessageBox.about(self, "Error", "Please choose mode!");
                return
        else:
            if len(sm4_plaintext) % 16 != 0:
                QMessageBox.about(self, "Error", "Plaintext len is not 16's bytes")
                return
            if self.mode == "ECB":
    ...
    def sm4_decrypt(self):
        ...
        if encrypt_str == "":
            QMessageBox.about(self, "Error", "Encrypt result is null");
            return
        if self.is_hex_string(encrypt_str) == False:
            QMessageBox.about(self, "Error", "Encrypt result is not hex string");
            return

        if len(encrypt_str) % 32 != 0:
            QMessageBox.about(self, "Error", "Encrypt result is not 16's bytes");
            return
        ...
        if dec_result != "":
            ...
            QMessageBox.about(self, "Error", "SM4 Decrypt failed !!!");
            self.result_textEdit.append("SM4 Decrypt Result: " + "SM4 Decrypt failed !!!")

    def sm3_hash_func(self):
        if self.plaintext_hex.text().strip() == "":
            self.msgbox_show("Error", "Plaintext is null")
            return
    ...
    def sm2_encrypt_func(self):
        public_key = self.pubkall_LE.text().strip()
        if len(public_key) != 128:
            QMessageBox.about(self, "Error", "Public key len is not 64 bytes")
            return
    ...
    def sm2_decrypt_func(self):
        ...
        if len(encrypt_str) <= 192:
            QMessageBox.about(self, "Error", "Encrypt result length should longer than 96 bytes");
            return
        ...
        if dec_data is None:
            ...
            QMessageBox.about(self, "Info", "SM2 Decrypt failed !!!");
        else:
    ...
    def sm2_verify_func(self):
        ...
        if self.is_hex_string(e_hash) == False:
            QMessageBox.about(self, "Error", "e_hash is not hex string");
            return
        ...
        if len(sign) != 128:
            QMessageBox.about(self, "Error", "Signature value, len is not 64 bytes")
            return
        if self.is_hex_string(sign) == False:
            QMessageBox.about(self, "Error", "Sign result is not hex string");
            return
        ...
        if verify == True:
            ...
            QMessageBox.about(self, "Info", "Falied to verify signature!!!");
